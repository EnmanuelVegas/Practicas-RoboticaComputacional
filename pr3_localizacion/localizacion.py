...
def localizacion(balizas, real, ideal, centro, radio, mostrar=False):
  ...
  # Utilizar np.arange para hacer un rango con números no enteros
  incremento = 0.1
  rango_x = np.arange(centro[0] - radio, centro[0] + radio, incremento)
  rango_y = np.arange(centro[1] - radio, centro[1] + radio, incremento)
  ...
  for y in rango_y:
    ...
    for x in rango_x:
      ideal.set(x, y, 0)
      idealMedidas = ideal.senseDistance(balizas)

      error = np.sqrt(np.mean((np.array(realMedida) - np.array(idealMedidas))**2))
      fila.append(error)
  ...
  ideal.set(xMejor, yMejor, realAngle)
...
objetivos = [[4.2, 4.2], [2.0, -3.5], [-4.0, 4.0], [-3.5, -0.5], [2.0, 1.5], [-2.5, -4.0]]
...
real = robot()
real.set_noise(0.02, 0.03, 0.15)  # Ruido lineal / radial / de sensado
real.set(*P_INICIAL)
...
localizacion(objetivos, real, ideal, [ideal.x, ideal.y], 3.0, MOSTRAR)
...
for punto in objetivos:
  while distancia(tray_ideal[-1],punto) > EPSILON and len(tray_ideal) <= 1000:
    ...
    # Decidir nueva localización
    medidasReales = np.array(real.senseDistance(objetivos))
    medidasIdeales = np.array(ideal.senseDistance(objetivos))

    differenceDistance = np.sqrt(np.mean((medidasReales - medidasIdeales)**2))
    if (differenceDistance > differenceThreshold):
      radio_busqueda = min(3.0, max(1.0, differenceDistance * 3.0))
      localizacion(objetivos, real, ideal, [ideal.x, ideal.y], radio_busqueda, MOSTRAR)
