...
while True:
  tit1, tit2, tit3, tit4 = tuple(p)
  # Parámetros D-H:
  #        0'     1      1'    2      3       4.1       4.2
  d  = [    5,   -2,     0,    2,     5,        0,        0]
  th = [ tit1,    0,  tit2,   90,  tit3,  tit4-90, -tit4-90]
  a  = [    0,    0,     3,    0,     0,        1,        1]
  al = [   90,    0,     0,   90,   -90,        0,        0]
  ...
  T00p=matriz_T(d[i],th[i],a[i],al[i])

  i += 1
  T0p1=matriz_T(d[i],th[i],a[i],al[i])
  T01=np.dot(T00p,T0p1)

  i += 1
  T11p=matriz_T(d[i],th[i],a[i],al[i])
  T01p=np.dot(T01,T11p)

  i += 1
  T1p2=matriz_T(d[i],th[i],a[i],al[i])
  ...
  i += 1
  T341=matriz_T(d[i],th[i],a[i],al[i])
  T041=np.dot(T03,T341)

  i += 1
  T342=matriz_T(d[i],th[i],a[i],al[i])
  ...
  # Puntos auxiliares
  o1p0 = np.dot(T01, [0, 0, 2, 1]).tolist()
  o2p0 = np.dot(T02, [0, -2, 0, 1]).tolist()

  # Mostrar resultado de la cinemática directa
  listaOrigenes = [o00, o1p0, o10, o2p0, o20, o30, [[o410], [o420]]]
  muestra_origenes(listaOrigenes)
  muestra_robot   (listaOrigenes, ef=ef0)
  entrada = input("Introduzca nuevas coordenadas: ")
  if (entrada == "exit" or entrada == "q" ):
    break
  p=[float(i) for i in entrada.split()]
